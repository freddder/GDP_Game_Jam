...
// *****************************************************
    ...
    GLint discardTexture_Location = glGetUniformLocation(program, "bDiscardTexture");
    GLint discardColor_Location = glGetUniformLocation(program, "bDiscardColor");
    GLint discardBlack_Location = glGetUniformLocation(program, "bDiscardBlack");

    if (pCurrentMesh->discardTexture)
    {
        glUniform1f(discardTexture_Location, (float)GL_TRUE);
        glUniform4f(discardColor_Location,
            pCurrentMesh->discardColor.r,
            pCurrentMesh->discardColor.g,
            pCurrentMesh->discardColor.b,
            pCurrentMesh->discardColor.a);
    }
    else
    {
    ...
    if (pCurrentMesh->discardBlack)
    {
        glUniform1f(discardBlack_Location, (float)GL_TRUE);
    }
    else
    {
    ...
    //GLint isInstanced_Location = glGetUniformLocation(program, "bDiscardTexture");
    ...
    if (pVAOManager->FindDrawInfoByModelName(pCurrentMesh->meshName, modelInfo))
    {
        ...
        if (pCurrentMesh->isInstanced)
        {
            glDrawElementsInstanced(GL_TRIANGLES,
                modelInfo.numberOfIndices,
                GL_UNSIGNED_INT,
                (void*)0,
                pCurrentMesh->instancedNumber);
        }
        else
        {
